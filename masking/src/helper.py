...
def pure_names_list(the_list, sep):
    ...
    assert (
        type(the_list) is list and type(the_list[0]) is not list
    ), "Input should be 1d list"
...
def make_dir_if_not_exists(directory, verbose=True):
    if not os.path.isdir(directory):
        ...
        if verbose:
            print(
                f'In [make_dir_if_not_exists]: created path "{directory}"'
            )  # do not import logger, or you'll get import error
...
def get_paths(model_name):
    ...
    return {
        "checkpoints_path": os.path.join(
            params["train"]["checkpoints_path"], model_name
        )
    }


def copy_files(
    source, dest, from_text_file=None, from_csv=None, sep=",", col="filename"
):
    assert (
        from_text_file or from_csv
    ), "Either a csv file or text file should be provided"
    if from_csv:
...
def show_num_params(model):
    ...
    globals.logger.info(
        f"Model total params: {total_params:,} - trainable params: {trainable_params:,}"
    )
...
def load_checkpoint(path_to_load, step, model, optimizer=None, resume_train=True):
    ...
    return (
        model.to(globals.get_current_device()),
        optimizer,
        loss,
        lr,
    )  # returned optimizer is None if not provided
