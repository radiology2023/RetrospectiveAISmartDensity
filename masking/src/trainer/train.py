...
def train(
    ...
    while epoch <= max_epoch:
        for i_batch, batch in enumerate(train_loader):
            ...
            logits = model(
                image_batch
            )  # forward uses ann_inds only if model was initialized with 'sep_anns' mode
            ...
            globals.logger.info(
                f"Epoch: {epoch}, step: {step}, batch: {i_batch + 1} - loss: {train_loss}"
            )
            if do_track:
            ...
            if step == 1 or step % eval_step == 0:
                ...
                if val_loader is not None:
                    model_for_eval = models.init_and_load_model_for_eval(
                        model_name,
                        loss_type,
                        step,
                        checkpoints_path=model_paths["checkpoints_path"],
                    )
                    val_dict = evaluation.calc_metrics(
                        val_loader, model_for_eval, loss_type
                    )
                    # track metrics
                    if do_track:
                        track_wandb_metrics(val_dict, step)
                    globals.logger.info(
                        f"Epoch: {epoch}, step: {step} => evaluation done\n"
                    )
