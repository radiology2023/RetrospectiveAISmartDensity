...
def to_str_list(tensor_or_list):
    assert (
        torch.is_tensor(tensor_or_list) or type(tensor_or_list) == list
    )  # input should be either tensor or list
...
def extract_batch(batch, loss_type):
    ...
    if "none" not in to_str_list(
        batch["label"]
    ):  # if there is 'none' in label, it means there is no label
        labels = batch["label"].to(
            globals.get_current_device()
        )  # label is always scalar for each image
        ...
        labels, targets = (
            batch["label"],
            batch["label"],
        )  # list of 'none' values in case there is no label
    ...
    with torch.no_grad():
        if loss_type == "multi_hot":
            ...
            for i in range(batch_size):  # for each item in batch
                thresholded_row = thresholded[
                    i, :
                ]  # apply threshold to probabilities to replace floats with either 1's or 0's
                predicted_label = scan_thresholded(
                    thresholded_row
                )  # scan from left to right and make the final prediction
...
def multi_hot_score(cdf_list):
    ...
    score = softmax_score(
        probs
    )  # now that we have prob for each bin, we can use the nice formula
    return probs, score
