...
def parse_args():
    ...
    parser.add_argument(
        "--train_folder", type=str
    )  # explicitly set it in the argument, if wanted
    ...
    parser.add_argument(
        "--cv", type=int
    )  # e.g. --cv 1 denoting the first fold of cross-validation
    parser.add_argument("--checkpoints_path", type=str)
    parser.add_argument("--line_parse_type", type=int)  # for csv file lines
    parser.add_argument(
        "--save_preds_to", type=str
    )  # path to save model predictions to
    ...
    parser.add_argument(
        "--train_augments", nargs="+"
    )  # data augmentations, passed as strings

    parser.add_argument(
        "--gpu_id", type=int, default=7
    )  # -1 for cpu (NOT IMPLEMENTED FOR NOW)
...
def check_args(args):
    ...
    assert (
        args.train or args.evaluate
    ), "main script should be run with either --train or --evaluate"
...
def train_model(args, params):
    ...
    if args.resume_step is not None:
        model, optimizer, _, _ = helper.load_checkpoint(
            path_to_load=helper.get_paths(model_name)["checkpoints_path"],
        ...
        globals.logger.info(
            f"Model and optimizer loaded successfully from step: {args.resume_step}"
        )

    # running with cross-validation
    if args.cv:  # determining files with cross validation
        globals.logger.info(
            f"\033[1mPerforming cross-validation for fold: {args.cv}\033[0m"
        )
        ...
        train_list = [
            line for line in all_train_list if line.split(";")[0] in train_names
        ]
    ...
    # usual running
        ...
        train_csv, val_csv = (
            params["data"]["train_csv"],
            params["data"]["val_csv"],
        )  # val_csv could be None
        train_list = helper.read_csv_to_list(train_csv)
        val_list = (
            helper.read_csv_to_list(val_csv) if val_csv is not None else []
        )  # final training, without any validation

    globals.logger.info(
        f"\033[1mCreated train_list with len: {len(train_list):,}, "
    ...
    common_dataset_params = {
        "data_folder": params["data"][
            "train_folder"
        ],  # same data_folder for both train and cross-val
    ...
    train_dataset_params = {
        ...
        "augments": params["train"]["augments"],
        **common_dataset_params,
    }
    train_data_loader_params = {"shuffle": True, **common_data_loader_params}
    train_loader = data_handler.init_data_loader(
        train_dataset_params, train_data_loader_params
    )
    ...
    if len(val_list) == 0:
        ...
        globals.logger.info(
            f"\033[1mInitialized train_loader of len: {len(train_loader)}, val_loader is: None \033[10m\n"
        )
    else:
        ...
        val_loader = data_handler.init_data_loader(
            val_dataset_params, val_data_loader_params
        )
        globals.logger.info(
            f"\033[1mInitialized train_loader of len: {len(train_loader)}, val_loader of len: {len(val_loader)}\033[10m\n"
        )
