...
def init_model(loss_type):
    ...
    globals.logger.info(
        f"Model is now being moved to device: {globals.get_current_device()}\n"
    )
...
def init_and_load_model_for_eval(model_name, loss_type, step, checkpoints_path=None):
    ...
    model = init_model(
        loss_type
    )  # always init with no pretraining because then we will load a checkpoint
    if (
        checkpoints_path is None
    ):  # meaning that set_globals has been called in the beginning of the program
        path = helper.get_paths(model_name)["checkpoints_path"]
    else:  # otherwise the folder containing checkpoints is provided
        path = checkpoints_path
    model = helper.load_checkpoint(path, step, model, resume_train=False)[
        0
    ]  # load mode in eval mode
    return model


def load_resnet(
    model_name, pretrained, freeze_params, n_layers_to_remove, verbose=False
):
    """
    ...
    resnet_model = torch.nn.Sequential(
        *list(resnet_model.children())[:-n_layers_to_remove]
    )
    ...
    if verbose:
        ...
        summary(
            resnet_model, input_size=(3, 316, 256)
        )  # feature maps of shape (512, 10, 8) for resnet18 and 34
        helper.waited_print("")
...
class Predictor(torch.nn.Module):
    def __init__(
        ...
        self.backbone = load_resnet(
            ...
            freeze_params=if_freezed,
            n_layers_to_remove=1,
        )  # resnet with average pooling
        self.fc_layer = nn.Linear(
            in_features=fc_in_channels, out_features=fc_out_channels
        )

    def forward(self, inp):
        backbone_out = (
            self.backbone(inp).squeeze(3).squeeze(2)
        )  # (N, C, 1, 1) -> (N, C)
