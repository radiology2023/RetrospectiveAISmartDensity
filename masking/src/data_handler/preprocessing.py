...
def get_breast_percent(
    img_array, if_pad_small_resolution=False, if_large_resolution=False
):
...
def get_text_contour(
    ...
    """
    _, array_bin = cv2.threshold(
        src=array, thresh=0, maxval=255, type=cv2.THRESH_BINARY
    )  # binarize image
    _, contours, _ = cv2.findContours(
        array_bin.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
    )
    ...
    array_breast_removed = cv2.drawContours(
        array.copy(), contours, breast_cont_idx, color=(0, 0, 0), thickness=cv2.FILLED
    )
    ...
    array_breast_removed = cv2.morphologyEx(
        src=array_breast_removed.copy(),
        op=cv2.MORPH_DILATE,
        kernel=cv2.getStructuringElement(shape=cv2.MORPH_RECT, ksize=kernel_size),
    )
    _, contours, _ = cv2.findContours(
        array_breast_removed.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
    )
    ...
    contours = [
        ...
        if min_contour_area < cv2.contourArea(cont) < max_contour_area
    ]
    ...
    if len(contours) > 0:
        ...
        for (
            ...
            breast_cont
        ):  # get the point with lowest x and highest y corresponding to breast
        ...
        contours_min_y = [
            min([copy.deepcopy(point).flatten()[1] for point in cont])
            for cont in contours
        ]
        ...
        i_cont_to_be_skipped = [
            i
            for i in range(len(contours))
            if contours_min_y[i] > breast_max_y_with_min_x
        ]
    ...
    if len(contours) > 0:
        contours_max_x = [
            max([copy.deepcopy(point).flatten()[0] for point in cont])
            for cont in contours
        ]
        i_selected_contour = contours_max_x.index(
            max(contours_max_x)
        )  # contour whose most right-hand side point is on the right of other contours
        contours = [contours[i_selected_contour]]

    assert (
        len(contours) <= 1
    ), f"There are more than one contour remaning for this image: {save_as}"  # there should remain one countour at most
    ...
    if save_as:
        if len(contours) == 1:
            if colorful:
                ...
                img_8u = cv2.cvtColor(
                    array, cv2.COLOR_GRAY2BGR
                )  # change to RGB so it shows the color
                thickness = cv2.FILLED
            else:
                color = (0, 0, 0)  # black
                img_8u = array
                thickness = cv2.FILLED
            img_8u = cv2.drawContours(
                img_8u.copy(), contours, contourIdx=-1, color=color, thickness=thickness
            )  # fill inside selected contour (contourIdx=-1 means all contours)
...
def segment_breast(img, threshold=0):
    ...
    _, img_bin = cv2.threshold(
        src=img, thresh=threshold, maxval=255, type=cv2.THRESH_BINARY
    )
    ...
    breast_mask = cv2.drawContours(
        np.zeros_like(img_bin), contours, idx, color=255, thickness=-1
    )  # -1 is the same as cv2.FILLED
...
def move_breast_centroid(img, centroid, full_height, full_width, if_movey=False):
    ...
    if centroid[0] >= (
        w - centroid[0]
    ):  # if centroid x in the right half of total width of image
        img_new = np.full(
            (h, 2 * centroid[0]), min_pixel_value
        )  # make a big pad of zero whose width is two times that of centroid x (so centroid x would be in the center)
        img_new[
            :, :w
        ] = img  # fill the left part of the big pad with the actual image (no change in h) - equivalent to right padding
    else:  # for smaller breasts
        img_new = np.full(
            (h, 2 * (w - centroid[0])), min_pixel_value
        )  # do the above steps in the opposite way
    ...
    if h > full_height:
        ...
        ]  # crop h in the center
    if w > full_width:
        img = img[
            :, int((w - full_width) / 2) : int((w - full_width) / 2) + full_width
        ]  # crop w in the center
...
def _save_as_preprocessed_png(
    ...
    for i_file, filename in enumerate(basenames):
        ...
        dicom_photometricinterpretation = row[
        ...
        img_array = exposure.rescale_intensity(
            ...
            np.uint8
        )  # by default output dtype is float64, we make it uint8
        ...
        new_img = move_breast_centroid(
            img=img_array,
            centroid=(cx, cy),
            full_height=desired_full_height,
            full_width=desired_full_width,
            if_movey=if_movey,
        )
        ...
        if filename not in specials["skip"]["filenames"]:
            ...
            for special_key in [
                key for key in specials.keys() if key.startswith("special")
            ]:
            ...
            contours_list = get_text_contour(
                new_img, **config
            )  # get the contour aroud text (list either empty or necessarily has one element)
            ...
            if len(contours_list) > 0:
                ...
                new_img = cv2.drawContours(
                    ...
                    color=min_val,
                    thickness=cv2.FILLED,
                )  # fill the contour with min value of the array (air)

        # sanity check that dtype is as desired
        assert (
            new_img.dtype == np.uint8
        ), f"new_img dtype is supposed to be {np.uint8} but is {new_img.dtype}"
        # make sure the output image dimensions are as desired
        assert (
            new_img.shape[0] == desired_full_height
            and new_img.shape[1] == desired_full_width
...
def raw_to_preprocessed(
    raw_folder,
    labels_path,
    save_dir,
    specials,
    desired_full_height=632,
    desired_full_width=512,
    if_movey=False,
...
def _save_as_raw_png(dicom_folder, dicom_basenames, png_folder, image_size):
    ...
    """
    assert (
        type(image_size) == tuple or type(image_size) == dict
    ), "image_size should either be a tuple or a dict"
    ...
    for i, dicom_basename in enumerate(dicom_basenames):
        ...
        if type(image_size) == tuple:
            ...
            assert (
                orig_shape_wh in image_size
            ), f"Down-sampling size for orig_shape={orig_shape_wh} not specified in image_size={image_size}"
            determined_image_size = image_size[orig_shape_wh]
        else:
            raise NotImplementedError(
                '"determined_image_size" cannot be calculated for the current image_size'
            )
        # save the raw PNG as 16-bit (16 bits allocated to dicom pixel_array)
        cv2.imwrite(
            new_path, cv2.resize(img_array.astype(np.uint16), determined_image_size)
        )
...
def dicoms_to_raw_pngs(
    dicom_folder, dicom_basenames, png_folder, image_size, n_processes=1
):
    ...
    print(
        f"Doing pre-processing for {len(dicom_basenames)} files, dicom_folder: {dicom_folder}"
    )
    if n_processes > 1:
        ...
        all_args = [
            (dicom_folder, the_chunk, png_folder, image_size) for the_chunk in chunks
        ]
        with multiprocessing.Pool(n_processes) as pool:
    ...
    for root, fodler, files in os.walk(dicom_path):
        for name in files:
            if name.endswith(".dcm"):
                ...
                df.loc[
                    i, "dicom_photometricinterpretation"
