...
class MammoDataset(data.Dataset):
    def __init__(
        ...
        globals.logger.info(
            f"MammoDataset created with image size: {img_size}, imread_mode: {imread_mode}\n"
        )

        # make data_list if not provided
        if self.data_list is None:
            self.data_list = helper.files_with_suffix(
                self.data_folder, suffix=".png", pure=True
            )
            globals.logger.info(
                f"Manually initialized data_list with all {len(self.data_list)} png images in data_folder"
            )
    ...
    def make_image_tensor(self, image_name):
        ...
        return train_preprocessing.load_and_preprocess(
            image_path, self.img_size, self.imread_mode, self.augments
        )[1]
    ...
    def __getitem__(self, index):
        ...
        if label != "none":
            label = train_preprocessing.convert_label(
                label, direction="to_train"
            )  # convert scalar labels from [1-8] to [1-7]
...
def init_data_loader(dataset_params, data_loader_params):
    if (
        "augments" in dataset_params.keys()
    ):  # val dataset params does not have 'augments'
